...
def create_game(board):
    """
    Creates a fully wired game session for integration tests.

    Mirrors GameFactory wiring but uses test-safe SimulationService assembly.
    """
    ...
    message_bus.subscribe(MovePerformedEvent, move_history_observer.handle)
    ...
    class PromotionEngineFacade:
        def __init__(self, engine, promotion_service):
            self._engine = engine
        ...
        def submit_pawn_promotion_choice(self, piece_id, chosen_kind):
            return self._promotion_service.submit_pawn_promotion_choice(
                piece_id,
                chosen_kind,
            )

    promotion_facade = PromotionEngineFacade(game_engine, pawn_promotion_service)

    move_request_handler = MoveRequestHandler(game_engine)
    jump_request_handler = JumpRequestHandler(game_engine)
    promotion_request_handler = PromotionRequestHandler(promotion_facade)

    message_bus.subscribe(MoveRequestedMessage, move_request_handler.handle)
    message_bus.subscribe(JumpRequestedMessage, jump_request_handler.handle)
